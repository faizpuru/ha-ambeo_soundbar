...
class AmbeoMediaPlayer(AmbeoBaseEntity, MediaPlayerEntity):
    ...
    def media_title(self):
        ...
        if self.coordinator.data and "player_data" in self.coordinator.data:
            track_roles = self.coordinator.data["player_data"].get("trackRoles", {})
    ...
    def state(self):
        ...
        power_state = STATE_DICT.get(self.coordinator.data.get("state"), STATE_ON)

        if power_state == STATE_ON and "player_data" in self.coordinator.data:
            player_state = self.coordinator.data["player_data"].get("state")
    ...
    def source_list(self):
        ...
        titles = [entry["title"]
                  for entry in self.coordinator.sources if "title" in entry]
    ...
    def sound_mode_list(self):
        ...
        titles = [preset["title"]
                  for preset in self.coordinator.presets if "title" in preset]
        return sorted(titles)
