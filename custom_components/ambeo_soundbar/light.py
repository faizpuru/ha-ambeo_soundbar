...
from .const import DOMAIN, DEFAULT_BRIGHTNESS, BRIGHTNESS_SCALE, BRIGHTNESS_SCALE_AMBEO_MAX_LOGO, BRIGHTNESS_SCALE_AMBEO_MAX_DISPLAY, DEFAULT_BRIGHTNESS_AMBEO_MAX
    ...
    async def async_turn_on(self, **kwargs):
        ...
        try:
            if ATTR_BRIGHTNESS in kwargs:
                self._brightness = math.floor(brightness_to_value(BRIGHTNESS_SCALE, kwargs[ATTR_BRIGHTNESS]))
            else:
...
class AmbeoMaxLogo(BaseLight):
    def __init__(self, device, api):
        super().__init__(device, api, "Ambeo Max Logo", "ambeo_max_logo", BRIGHTNESS_SCALE_AMBEO_MAX_LOGO)

    async def async_turn_on(self, **kwargs):
        """Turn on the Ambeo Max Logo with specified brightness, if provided, otherwise use default brightness."""
        try:
            if ATTR_BRIGHTNESS in kwargs:
                self._brightness = math.floor(brightness_to_value(BRIGHTNESS_SCALE_AMBEO_MAX_LOGO, kwargs[ATTR_BRIGHTNESS]))
            else:
                self._brightness = DEFAULT_BRIGHTNESS_AMBEO_MAX
            await self.api.set_ambeo_max_logo_brightness(self._brightness)        
        except Exception as e:
    ...
    async def async_turn_off(self, **kwargs):
        ...
        try:
            await self.api.set_ambeo_max_logo_brightness(0)  
            self._brightness = 0
        except Exception as e:
    ...
    async def async_update(self):
        ...
        try:
            brightness = await self.api.get_ambeo_max_logo_brightness()
            self._brightness = brightness
        except Exception as e: 
...
class AmbeoMaxDisplay(BaseLight):
    def __init__(self, device, api):
        super().__init__(device, api, "Ambeo Max Display", "ambeo_max_display", BRIGHTNESS_SCALE_AMBEO_MAX_DISPLAY)

    async def async_turn_on(self, **kwargs):
        """Turn on the Ambeo Max Display with specified brightness, if provided, otherwise use default brightness."""
        try:
            if ATTR_BRIGHTNESS in kwargs:
                self._brightness = math.floor(brightness_to_value(BRIGHTNESS_SCALE_AMBEO_MAX_DISPLAY, kwargs[ATTR_BRIGHTNESS]))
            else:
                self._brightness = DEFAULT_BRIGHTNESS_AMBEO_MAX
            await self.api.set_ambeo_max_display_brightness(self._brightness)        
        except Exception as e:
    ...
    async def async_turn_off(self, **kwargs):
        ...
        try:
            await self.api.set_ambeo_max_display_brightness(0)  
            self._brightness = 0
        except Exception as e:
    ...
    async def async_update(self):
        ...
        try:
            brightness = await self.api.get_ambeo_max_display_brightness()
            self._brightness = brightness
        except Exception as e: 
